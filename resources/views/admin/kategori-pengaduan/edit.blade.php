{{-- resources/views/admin/kategori-pengaduan/edit.blade.php --}}
@extends('admin.app')
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3" style="background-color: #800000;">
                <h6 class="m-0 font-weight-bold text-white">
                    <i class="fas fa-edit"></i> Edit: {{ $kategoriPengaduan->nama_kategori }}
                </h6>
            </div>
            <div class="card-body">
                <form action="{{ route('kategori-pengaduan.update', $kategoriPengaduan->id_kategori) }}" method="POST">
                    @csrf
                    @method('PUT')
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <strong>{{ $kategoriPengaduan->created_at->format('d M Y, H:i') }}</strong>
                </div>
                
                <div class="mb-3">
                    <small class="text-muted">Terakhir Diubah:</small><br>
                    <strong>{{ $kategoriPengaduan->updated_at->format('d M Y, H:i') }}</strong>
                </div>
                
                <div class="mb-0">
                    <small class="text-muted">Jumlah Pengaduan:</small><br>
                    <span class="badge badge-info badge-pill">
                        {{ $kategoriPengaduan->pengaduan->count() }} Pengaduan
                    </span>
                </div>
            </div>
        </div>
